...
-- Drop in dependency order (children before parents) in case of re-run
...
-- ---- Sample Data: Learners ----
-- Note: Emma Clarke (id 6) is included deliberately with NO purchases,
-- so the LEFT JOIN in Section 2 has a real unmatched row to demonstrate.
...
-- ---- Sample Data: Courses ----
-- Course 105 (Cloud Architecture Mastery) is deliberately never purchased,
-- so it can be used to demonstrate RIGHT JOIN / Q5 / Q11 (NULL handling).
...
-- Emma Clarke will appear with NULLs in the course/quantity/amount columns.
SELECT
    l.full_name                      AS learner_name,
    c.course_name                    AS course_name,
    c.category                       AS category,
...
-- Cloud Architecture Mastery will appear with NULLs in learner/quantity/amount.
SELECT
    l.full_name                      AS learner_name,
    c.course_name                    AS course_name,
    c.category                       AS category,
...
-- (derived-table subquery in the FROM clause + scalar subquery in the WHERE clause)
SELECT
    l.full_name AS learner_name,
...
-- (multi-row subquery using the ANY operator)
SELECT
...
-- (correlated subquery - the inner query re-evaluates per outer row's country)
SELECT
    l.full_name AS learner_name,
    l.country   AS country,
    FORMAT(SUM(p.quantity * c.unit_price), 2) AS total_spending
...
SELECT
    ...
    -- IFNULL(SUM(p.quantity), 0) would work identically in MySQL
FROM courses c
LEFT JOIN purchases p ON c.course_id = p.course_id
GROUP BY c.course_id, c.course_name, c.category
